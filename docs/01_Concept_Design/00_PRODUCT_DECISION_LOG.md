...
2026-09-13 결정 7·재검토 조건(본 문서 962행, 994행)은 로그인을 카카오
단일 제공자로 좁히는 것을 의도된 콜드 스타트 전략으로 명시했고, "해외
사용자 확보가 실제 목표가 되는 시점"을 재검토 트리거로 못박았다. 그
전까지 Google 등 다른 제공자를 추가하는 것은 이 문서 기준으로 결정
위반이었다(`docs/04_Logic_Progress/00_BACKLOG.md:441`도 동일하게
"Google 등 다른 제공자는 추가하지 않는다"를 구현 전제조건으로 명시).
이제 해외 확장이 실제 목표로 확정되어, 재검토 조건이 충족됐다.

### 결정

1. 로그인 제공자로 Google을 카카오와 함께 추가한다. 카카오 단일 제공자
   원칙(2026-09-13 결정 7)은 폐기하지 않고, "해외 확장 전까지 유지"라는
   조건이 이제 지났다고 갱신한다 — 카카오 지원 자체를 없애는 결정은
   아니다.
2. DB 스키마는 이미 provider-agnostic하다(`oauth_accounts` 테이블의
   `provider` + `provider_account_id` 복합키, `packages/db/src/schema.ts`) —
   마이그레이션 없이 `provider = "google"` 행을 추가하는 방식으로
   확장한다. 표시 이름과 로그인 식별자를 분리하는 원칙(2026-09-13 결정
   8)과 로그인 계정 ID·이메일·전화번호를 노출하지 않는 원칙은 제공자에
   관계없이 그대로 적용한다.
3. 동일 인물이 카카오와 Google 양쪽으로 각각 로그인하면 서로 다른
   participant로 취급한다(계정 병합 기능은 이번 결정 범위 밖) — 기존
   카카오 participant 식별 방식(`findOrCreateKakaoParticipant`)과 같은
   패턴으로 Google 전용 조회/생성 경로를 추가하되, 두 provider를 하나의
   participant로 합치는 로직은 만들지 않는다.

### 관련 문서

- 이 결정으로 아래 문서의 "카카오만 제공한다" 서술이 갱신 대상이 된다:
  `docs/04_Logic_Progress/00_BACKLOG.md:84`, `:441`,
  `docs/02_UI_Screens/02_INVITE_GRAPH_CONCEPT.md:23`, `:120`,
  `docs/03_Technical_Specs/03_INVITE_GRAPH_V2_SPEC.md` §4 "카카오 단일
  제공자 결정".
- 본 문서 2026-09-13 항목 결정 7·재검토 조건(962행, 994행)

## 2026-09-16 — "건너건너 이어지는 중간 사람의 정보는 공개되지 않아요" 안심 문구 (구현 완료)
...
중간 연결자의 identity를 절대 노출하지 않는다는 원칙(`AGENTS.md` §1 원칙
4)은 처음부터 지키고 있었지만, 그 사실이 코드 주석에만 있었고 실제로
사용자가 참여를 결정하는 화면(`/upload`, `/t/{token}`의 "나도 연결
보태기")에는 한 번도 보인 적이 없었다. 반대로 회원가입 화면의 "길의
마지막 연결자가 되면 이 이름이 챌린지에 표시될 수 있어요"는 노출 위험을
경고하는 문구라, 균형을 맞추는 안심 문구 없이 이것만 먼저 보면 오히려
참여를 더 망설이게 만들 수 있다. "사생활이 걱정돼서 아예 연결을 보태지
않는 사람도 있을 것"이라는 가설에서 나온 결정이다.

### 결정

1. `/upload`의 두 화면(챌린지에서 진입하는 intro 화면, 기본 업로드
   화면), `/t/{token}`의 "나도 연결 보태기" 두 분기(데이터가 이미 있는
   경우/없는 경우), 그리고 홈(`/`)의 "생각보다 가까울지도 몰라요" 카드
   모두에 "건너건너 이어지는 중간 사람의 정보는 공개되지 않아요."를
   짧은 캡션으로 추가한다. 홈 카드에서는
   본문보다 확실히 작고 연한 글자로 둔다 — 이 카드의 핵심 주장("생각보다
   가까울지도 몰라요")을 방해하지 않으면서, 아직 아무것도 결정하지 않은
   첫 방문자에게도 이 원칙을 가장 먼저 보여준다. 같은 김에 홈 카드의
   본문도 "여럿이 아는 관계를 하나씩 보태면 → 아는 사이가 모이면 /
   궁금한 사람까지 이어지는 길을 찾을 수 있어요 → 건너건너 이어질 수
   있어요"로 다듬어 브랜드 표현("건너건너")과 맞춘다. 기존 "서로 팔로우하는
   사람만 연결에 사용해요." 류의 데이터-사용 안내와는 별개의 문장으로
   둔다 — 하나는 "무엇을 쓰는지", 하나는 "내 신원이 드러나는지"에 대한
   답이라 서로 다른 질문에 답한다.
2. **"중간에 있는 사람"과 "마지막 연결자"는 이 문구에서 서로 다른
   범주로 구분해 다룬다.** "중간 사람"은 target 바로 직전이 아닌 모든
   구간을 가리키고, 이 구간은 예외 없이 항상 익명이다(동의 여부와
   무관, 우회 경로 없음). target 바로 직전(마지막 연결자)은 이 문구가
   다루는 대상이 아니다 — 그 경우는 회원가입 때 이미 별도로 고지하고
   동의를 받은 사람의 표시 이름만 최대 3명까지 조건부로 보일 수 있다는
   기존 규칙을 그대로 유지한다. 두 문구는 모순이 아니라 서로 다른
   구간에 대한 서로 다른 진실을 말한다.
3. 개인정보처리방침(`/privacy`)에 같은 약속을 "2. 연결 정보의 공개"
   절로 정식 추가한다 — 화면 카피와 법적 문서가 서로 다른 말을 하는
   불일치를 만들지 않기 위해서다. 같은 절에서 Instagram 업로드 관련
   처리(맞팔만 사용, 원본 파일 미저장, 아이디는 해시로 처리)도
   처음으로 명시한다 — 실제 핵심 기능인데 방침에는 지금까지 전혀
   언급이 없던 공백이었다.

### 관련 구현

- `apps/web/components/InstagramImportFlow.tsx`
- `apps/web/components/TargetChallengeScreen.tsx`
- `apps/web/app/privacy/page.tsx`
- `apps/web/app/page.tsx`(홈 "생각보다 가까울지도 몰라요" 카드)
- `apps/web/app/globals.css`(`.landing-basis-privacy-note`)
...
페이지뷰·체류·이탈은 `layout.tsx`의 gtag 기본 설정이 이미 잡는다. 공개
챌린지 발견 퍼널은 그것만으로 끊기므로(예: `/t/{token}` 페이지뷰만으로는
공개 목록에서 왔는지 공유 링크로 왔는지 알 수 없다) 아래 이벤트를 추가했다.

| 이벤트 | 시점 | 파라미터 |
| :-- | :-- | :-- |
| `challenge_list_open` | `/challenges`로 진입 | `source`: `home` / `my_page` |
| `challenge_open` | 목록에서 챌린지 항목 클릭 | `source`: `list` |
| `my_page_open` | 헤더 "마이페이지" 클릭 | — |
| `challenge_contribute_click` | "나도 연결 보태기"/"아는 사람 더 가져오기" 클릭 | `source`: `challenge` / `rejoin` |
| `challenge_share` | 챌린지 공유/복사 **완료** | `status`: `searching` / `found` |

`challenge_contribute_click`이 따로 필요한 이유는 기존 `challenge_join`이
업로드를 **끝냈을 때만** 찍히기 때문이다 — 버튼을 누르고 `/upload`에서
이탈한 사람이 그 사이에 통째로 사라진다. `challenge_share`는 공유 시트를
취소하면 보내지 않는다("눌렀다"가 아니라 "실제로 공유까지 갔다"만 센다).

식별 정보는 어떤 파라미터에도 넣지 않는다 — 챌린지 토큰조차 보내지 않고,
어느 화면에서 눌렀는지(`source`)까지만 보낸다. 서버 컴포넌트 안에서도
클릭을 집계할 수 있도록 얇은 클라이언트 래퍼(`TrackedLink`)를 하나만 두고,
각 화면을 클라이언트 컴포넌트로 바꾸지 않는다.

### 챌린지별 공유 횟수는 GA가 아니라 DB에 쌓는다(2026-09-15 추가 결정)

"어느 챌린지가 몇 번 공유됐는지"를 알고 싶다는 요청에서 시작했다. GA4의
`challenge_share`는 위 표대로 챌린지 식별자를 전혀 담지 않으므로 전체
합산만 알 수 있고 챌린지별로는 구분할 수 없다 — 의도적으로 그렇게
설계했으니, 이걸 깨고 토큰이나 `displayName`을 이벤트 파라미터에 실어
보내는 방향은 채택하지 않았다. 비공개 챌린지의 `display_name`은 일반인
이름일 수 있고, 그러면 그 이름이 구글(GA) 서버로 그대로 넘어간다.

대신 `target_challenges.share_count`(신규 컬럼)를 두고, 공유가 실제로
완료됐을 때(`POST /api/challenges/{token}/share`, 로그인 불필요) 서버에서
1 증가시킨다. **운영자 전용 지표다** — `challengePublicResultSchema`를
비롯해 어떤 공개 API 응답에도 이 값을 싣지 않는다. 화면에 "12회 공유됨"
같은 걸 보여주려는 게 아니라, 운영자가 SQL로 직접 조회해서만 보는
용도이기 때문이다(`SELECT display_name, share_count FROM
target_challenges ORDER BY share_count DESC;`). 사용자용 조회 API나 관리자
대시보드는 만들지 않았다 — `is_public` 토글과 같은 운영 패턴(직접 SQL)을
그대로 따른다.
...
7. **동일 사용자를 하나의 그래프 노드로 식별한다.** 로그인은 카카오만 제공한다 —
   ...
   **[2026-09-16 갱신]** 재검토 조건이 충족되어 Google을 추가한다 — 아래
   "2026-09-16 — 로그인 제공자에 Google 추가" 항목 참고.
...
- 해외 사용자 확보가 실제 목표가 되는 시점이 오면, 로그인 제공자를 카카오 외에도
  ...
  **[2026-09-16 충족]** 해외 확장이 실제 목표로 확정되어 Google을 추가했다 —
  "2026-09-16 — 로그인 제공자에 Google 추가" 항목 참고.
