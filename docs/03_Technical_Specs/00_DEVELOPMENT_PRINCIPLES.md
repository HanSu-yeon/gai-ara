...
5. **지인 전용 초대 링크로 상대가 직접 확인했을 때만 그래프에 edge가 생긴다.**
   공개(탐색) 링크 방문은 몇 명이 방문하든 edge를 만들지 않는다 — 방문 그래프와
   인맥 그래프를 섞지 않는다. Instagram 팔로우 데이터는 edge 생성에 쓰지 않는다
   (2026-09-13 결정, 후순위 보류). 근거와 재검토 조건은
   [00_PRODUCT_DECISION_LOG.md](../01_Concept_Design/00_PRODUCT_DECISION_LOG.md)의
   2026-09-13 항목 참고. 그래프 저장 구조는 TASK-002(2026-09-13)에서 이 결정에 맞춰
   재설계됐다 — `pair_invites.status='accepted'` 행이 유일한 활성 edge 소스이고,
   기존 맞팔(`follows`) 기반 로직은 코드상 보존되지만 비활성이다. 상세는
   [01_DB_SCHEMA.md §4](./01_DB_SCHEMA.md#4-그래프-저장-구조-지인-확인-기반-edge--instagram-맞팔은-비활성-보존) 참고.
