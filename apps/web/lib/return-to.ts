const RETURN_TO_TOKEN_PATTERN = /^\/(?:invite|r)\/[A-Za-z0-9_-]+$/;
const RETURN_TO_EXACT_PATHS = new Set(["/connect", "/result"]);

/**
 * 로그인 뒤 돌아갈 수 있는 내부 경로만 명시적으로 허용한다(allowlist) —
 * 토큰형 경로(`/invite/{token}`, `/r/{token}`)와 로그인 사용자 전용 화면
 * (`/connect`, `/result`) 두 종류뿐이다. 이 목록에 없는 값은(외부 URL,
 * `//`로 시작하는 프로토콜 상대 경로, 알 수 없는 내부 경로 포함) 전부
